...
/* function getcomps(projectId,unitId)
{
	...
	function deletedailyInfo(inputId){
		
		
		document.forms[0].urlvalue.value="../comptsimis/deleteCompdailyInput";
		
		 
		   $("#compInputId").val(inputId);
		   $("#deleteFlag").val(true);
...
function getcompdailyinfo(compId,unitId)


{
	
	document.getElementById("info").innerHTML = " ";
	//alert("kalcoming"+compId);
	//alert(unitId);
	var m=  $("#componentDate").val();
	//alert("m"+m);
	 $("#deleteFlag").val(false);
	 $("#unitId").val(unitId);
	 $("#componentId").val(compId);
	  if(m==null||m ===''||typeof m === "undefined"){
		...
		document.getElementById("info").innerHTML = "Please Select Component Date";	
			return false;
	...
	function editdailyData(compInputId){
		 $("#editFlag").val(true);
		
		 var features="width=500,height=300,menubar=no,status=no,location=no,toolbar=no,scrollbars=yes,top=100,left=50";
	 	  	var url="../comptsimis/editCompDailyInput?compInputId="+compInputId+"&editFlag="+false;
...
<c:forEach items="${unitList}" var="u">								
	...
	<a class="myunit"  href="../comptsimis/deleteCompdailyInput?unitId=<c:out value="${u.unitId}"></c:out>" ><strong><c:out value="${u.unitName}"></c:out></strong></a>
	...
	<c:forEach items="${compsList}" var="d">
	 <c:if test="${c.projectId==d.projectId}"> 
	  <input name=componentId    id="componentId"  type=hidden  value="<c:out value="${d.componentId}"></c:out>" />
		...
		<li><a class="mycomps"  onclick="getcompdailyinfo(<c:out value="${d.componentId}"></c:out>,<c:out value="${u.unitId}"></c:out>)"  href="javascript:void(0);"><c:out value="${d.componentName}"></c:out></a>
	...
	<div class="panel panel-primary  col-md-12">
			<div class="panel-heading">
				<h3 class="panel-title">Edit/Delete Component Daily Input</h3>
			</div>
		...
		<div class="col-md-2">
				<a href="../comptsimis/deleteCompdailyInput">	<input type="button" class="btn btn-1 btn-primary center-block"
...
<div class="alert alert-info">
  <strong>Note:</strong> First Select Date and then Click on PumpHouse To Edit/Delete Data
</div>
  ...
  <strong id="info"></strong> 
				...
				<div class="col-md-12">	
				<div class="col-md-4">
				 <label class="label-control rowlebel ">Select Component Date </label>
				
				</div>
				<div class="col-md-4">
				
				<input name=componentDate   id="componentDate"  type=text  class="form-control compDate datepicker" readonly="readonly"  />	
				</div>
				<div class="col-md-4">
				</div>
				</div>
	...
	<!-- <div class="w3layouts_vertical_tab resp-tab-content resp-tab-content-active  col-md-8" style="display:block"> -->
		...
		<br></br>
									...
									<div class="col-md-1">Component Name</div>
									<div class="col-md-2">Pump Name</div>
									<div class="col-md-2">Pump Discharge</div>
									
									<div class="col-md-1">Pump Energy</div>
								...
								<div class="col-md-12"  style="color: blue">
										...
										<br></br>
											...
											<div class="col-md-1"><c:out value="${t.componentName}"></c:out></div>
											<div class="col-md-2"><c:out value="${t.pumpName}"></c:out>  </div>
											
											<div class="col-md-2"><c:out value="${t.pumpDischarge}"></c:out>  </div>
											<div class="col-md-1"><c:out value="${t.pumpEnergy}"></c:out> </div>
											<div class="col-md-2"><c:out value="${t.pumpStartTime}"></c:out>  </div>
											<div class="col-md-1"><c:out value="${t.pumpEndTime}"></c:out> </div>
											<div class="col-md-1"><c:out value="${t.pumpStartDate}"></c:out> </div>
										
											
											<div class="col-md-1">  <button type="button" class="btn btn-primary btn-sm" onclick='editdailyData(<c:out value="${t.compInputId}"></c:out>)' ><span class="glyphicon glyphicon-plus"></span></button>
											
							          <button type="button" class="btn btn-danger btn-sm" onclick='deletedailyInfo(<c:out value="${t.compInputId}"></c:out> )'><span class="glyphicon glyphicon-remove"></span></button></div>
