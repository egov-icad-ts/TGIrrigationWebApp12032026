...
public class PDFPACRecommondations extends AbstractITextpdfPortraitView {
	...
	//	listParas=(CAGParas) model.get("cagparas");
		...
		doc.addTitle("CAG PAC Paras Report");
		 Paragraph paragraphOne = new Paragraph("CAG PAC Paras Report", blackFont);
 ...
 String docname=" ";
	      ...
	      //	 doc.newPage();
           ...
           // System.out.println(m+"m"+e.PARAGRAPH);
            ...
            int c=listreplyData.size();
	        		...
	        		if((listparaData.get(a).getCagParaId()==listreplyData.get(d).getCagParaId())&& listreplyData.get(d).getCreateReplybyUnit()==9832 ){
      ...
      if( listreplyData.get(d).getReplyRemarks()!=null  ){
    	  ...
    	  if(listreplyData.get(d).getReplyRemarks().length()>2){
    		 ...
    		 // System.out.println("pacrecommendations"+listreplyData.get(d).getReplyRemarks().length()+"   "+listreplyData.get(d).getReplyRemarks());
