	...
	<form:form id="form20docs" method="post" modelAttribute="form20docsview" target="_blank" >
		...
		<div class="panel panel-primary " style="padding:10px">
			<div class="panel-heading">
				<h3 class="panel-title"> After Verification of <c:if test="${resgeninfo!=null }">
			
				<c:out value="${resgeninfo[0].freqoperationName}"></c:out> 
				<c:out value="${resgeninfo[0].year}"></c:out> 
				
				</c:if>
			
				  <c:if test="${tankgeninfo!=null }">
				
				
				<c:out value="${tankgeninfo[0].freqoperationName}"></c:out> 
				 <c:out value="${tankgeninfo[0].year }"></c:out> 
				  </c:if>  Remarks
							...
							<table class="display table-bordered " style="width:100%"  id="table1" >
								<thead >								
								  ...
								  <th style="color:black"><b>Sl. No</b></th>
								   <th style="color:black"><b>Remarks by </b></th>
								  <th style="color:black"><b>Inspection Office</b></th>
								    <th style="color:black"><b>Remarks</b></th>					  
								 
								  <th style="text-align: center;"><b>Over all condition of the Dam</b></th>
								  <th style="text-align: center;"><b>Category of the Dam</b></th>
								   <th style="text-align: center;"><b>Submitted to</b></th>
								...
								<tbody>
										...
										<c:forEach items="${resRemarksList}" var="t" varStatus="count">
										
											 <tr >
								  <td style="text-align: center;"><b>${count.count}</b></td>
								  
								 <td style="text-align: center;"><b>${t.remarksSubmittedBy}</b></td>
						
								  <td style="text-align: center;"><b>${t.inspectOfficeName}</b></td>
								
								  <td style="text-align: center;" ><b>${t.remarks}</b></td>
								  <td style="text-align: center;" ><b>${resgeninfo[0].totalDamCondition}</b></td>
								  
								  <c:if test="${resHealthInfo!=null }">
								  <c:forEach items="${resHealthInfo}" var="k" varStatus="kcount">
								  <c:if test="${k.latestHealthCategory==true}">
								 <c:choose>
								 <c:when test="${k.latestHealthCategory==true}">
								  <td style="text-align: center;" ><b>${k.deficiencyCategoryName}</b></td>
								  </c:when>
								  <c:otherwise>
								  <td style="text-align: center;" ><b></b></td>
								   ...
								   <td style="text-align: center;"><b>${t.submittedToEmpName}</b></td>
                 ...
                 
{
	extend : 'excelHtml5',
	autoFilter : true,
	sheetName : 'Exported_Remarks_data',
	text : 'Save To Excel',
	className : 'btn-primary ',
	messageTop : 'Inspection Remarks'
