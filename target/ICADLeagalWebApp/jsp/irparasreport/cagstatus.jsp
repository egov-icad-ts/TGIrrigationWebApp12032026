											...
											<%-- 	<td ><b><c:out value="${t.unitName}"></c:out></b></td> --%>
											 <td><button type="button" name="captank" id="paranum${count.count}" class="btn btn-1 btn-primary center-block "    onclick="getcagparadatails(0,false,'${t.cagGistId}','No','No')"  ><b><c:out value="${t.totalParas}"></c:out></b></button></td>
											  ...
											  <button type="button" name="captank" id="paranum${count.count}" class="btn btn-1 btn-primary center-block "    onclick="getcagparapacpdf('${t.cagGistId}')"  ><b>PAC recommendations with ATR</b></button>
												...
												<td><table>
												<c:if test="${paradocList!=null}">
												<c:forEach items="${paradocList}" var="k" varStatus="count">
											
												<c:if test="${t.cagGistId==k.cagGistId}">
												<tr><td>
												
												<a href="${pageContext.request.contextPath}/parasreport/viewCAGParas?fileName=${k.paraDocName}" target="_blank" > ${k.paraDocOriginalName}</a>
												<%-- <button type="button" name="captank" id="paranum${count.count}" class="btn btn-1 btn-primary center-block "  onclick="../uploadedFiles/CAGParas/${k.paraDocName}"   ><b>${k.paraDocOriginalName}</b></button> --%>
												</td></tr>
												</c:if>
												</c:forEach>
												</c:if></table></td>
