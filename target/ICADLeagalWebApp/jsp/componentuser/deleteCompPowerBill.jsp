...
function getcompmonpower(projectId)
{
	...
	document.forms[0].urlvalue.value="../comptsimis/deleteCompPowerBill";
...
function getcompmonpowerinfo(compId,unitId){
	...
	function deletePowerData(compPowerBillId){
		
		
		// alert("klcoming here"+compPowerId);
		 $("#deleteFlag").val(true);
		  $("#compPowerBillId").val(compPowerBillId);
	...
	function editPowerData(compPowerBillId){
		 $("#editFlag").val(true);
		
		 var features="width=500,height=300,menubar=no,status=no,location=no,toolbar=no,scrollbars=yes,top=100,left=50";
	 	  	var url="../comptsimis/editCompPowerBill?compPowerBillId="+compPowerBillId+"&editFlag="+false;
...
<form:form id="deleteCompMonPower" method="post"
				...
				<input name=compPowerBillId  id="compPowerBillId" type=hidden >
...
<c:forEach items="${unitData}" var="u">								
	...
	<a class="myunit"  href="../comptsimis/deleteCompPowerBill?unitId=<c:out value="${u.unitId}"></c:out>" ><strong><c:out value="${u.unitName}"></c:out></strong></a>
	...
	<div class="panel panel-primary  col-md-12">
		...
		<div class="col-md-2">
				<a href="../comptsimis/editCompPowerBill">	<input type="button" class="btn btn-1 btn-primary center-block"
				...
				<c:out value="${cppower[0].componentName}"></c:out> 
								...
								<div class="col-md-12"  >
									<c:forEach items="${cppower}" var="t" varStatus="count">
									 ...
									 <div class="row show-grid"  style="border-style: dashed; border-color: gray ;" >
									 
									
										
											
										<div class="col-md-3">S.No</div>	<div class="col-md-3" style="color: blue"><c:out	value="${count.count}"></c:out></div>											
										
										<div class="col-md-3">Bill Number</div>	<div class="col-md-3" style="color: blue"> <c:out value="${t.billNumber}"></c:out></div>
											
											
											
										<div class="col-md-3">Additional Security Deposite</div>	<div class="col-md-3" style="color: blue"><c:out value="${t.additionalSecurityDeposit}"></c:out> </div>
										<div class="col-md-3">Consumed Units</div>	<div class="col-md-3" style="color: blue"><c:out value="${t.consumedUnits}"></c:out> </div>
										
										<div class="col-md-3">Variable Charges</div>	<div class="col-md-3" style="color: blue"><c:out value="${t.variableCharges}"></c:out> </div>
										 <div class="row "  >	
										<div class="col-md-3">Fixed  Charges</div>	   <div class="col-md-3" style="color: blue"><c:out value="${t.fixedCharges}"></c:out>  </div>		
										</div>		
											
										
											
										<div class="col-md-3">Late Pay Surcharge</div>	<div class="col-md-3" style="color: blue"><c:out value="${t.latePaySurCharge}"></c:out> </div>
										<div class="col-md-3">ACD Surcharges</div>	<div class="col-md-3" style="color: blue"><c:out value="${t.ACDSurCharges}"></c:out> </div>
										<div class="col-md-3">Interest on security Deposite</div>	<div class="col-md-3" style="color: blue"><c:out value="${t.interestSecurityDeposite}"></c:out> </div>
										<div class="col-md-3">Total Amount</div>	<div class="col-md-3" style="color: blue"><c:out value="${t.totalAmount}"></c:out> </div>
										<div class="col-md-3">Paid Status</div>	<div class="col-md-3" style="color: blue">
											<c:if test="${t.paidStatus=='NP'}">
											Not paid
											</c:if>
											<c:if test="${t.paidStatus=='PP'}">
											Partial Payment
											</c:if>
											<c:if test="${t.paidStatus=='FP'}">
											Final Payment
											</c:if></div>
										<div class="col-md-2">View Bill</div>	<div class="col-md-2"><a target="_blank" href="../uploadedFiles/ComponentsPowerBill/${t.documentName}"><c:out value="${t.documentName}"></c:out></a> </div>
										<div class="col-md-3">Edit/Delete</div>	<div class="col-md-3">  <button type="button" class="btn btn-primary btn-sm" onclick='editPowerData(<c:out value="${t.compPowerBillId}"></c:out>)' ><span class="glyphicon glyphicon-plus"></span></button>
											
							          <button type="button" class="btn btn-danger btn-sm"  onclick='deletePowerData(<c:out value="${t.compPowerBillId}"></c:out> )' ><span class="glyphicon glyphicon-remove"></span></button></div>
