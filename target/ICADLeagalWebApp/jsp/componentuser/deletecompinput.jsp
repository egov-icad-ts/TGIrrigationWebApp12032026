...
/* function showpumpEdit(compInfoId){
	alert("compInfoId"+compInfoId);
	...
	$('#wrapper').dialog({
        ...
        open: function ()
        {
            ...
            $(this).load("../comptsimis/editCompInput?compInfoId="+compInfoId+"&editFlag="+false);
        },  
       
       
	    title: 'Edit Motor/Pump Component Info'
...
function getcompinputdata(projectId,c,u)
{
	//alert("kalcoming"+projectId);
	
	document.forms[0].urlvalue.value="../comptsimis/delCompInput";
	...
	/* 
	function deleteCompInputData(componentPowerAyacutId){
		
		
		alert("klcoming here"+componentPowerAyacutId);
		 $("#deleteFlag").val(true);
		  $("#componentPowerAyacutId").val(componentPowerAyacutId);
...
/* 	function editCompInputData(componentPowerAyacutId){
		 $("#editFlag").val(true);
		
		 var features="width=500,height=300,menubar=no,status=no,location=no,toolbar=no,scrollbars=yes,top=100,left=50";
	 	  	var url="../comptsimis/editCompInput?componentPowerAyacutId="+componentPowerAyacutId+"&editFlag="+false;
...
function deleteCompMotorData(compInfoId){
		
		
		
		 $("#deleteFlag").val(true);
		  $("#compInfoId").val(compInfoId);
	...
	function editCompMotorData(compInfoId){
		 $("#editFlag").val(true);
		
		 var features="width=500,height=300,menubar=no,status=no,location=no,toolbar=no,scrollbars=yes,top=100,left=50";
	 	  	var url="../comptsimis/editCompInput?compInfoId="+compInfoId+"&editFlag="+false;
...
<div class="w3_agile_main_left" id="delcompForm">
		<!--start left block-->
	 
<form:form id="delCompInput" method="post"
				modelAttribute="delCompInput" >
			...
			<!-- <input name=componentPowerAyacutId  id="componentPowerAyacutId" type=hidden > -->
				<input name=compInfoId  id="compInfoId" type=hidden >
...
<c:forEach items="${unitData}" var="u">								
	...
	<a class="myunit"  href="../comptsimis/delCompInput?unitId=<c:out value="${u.unitId}"></c:out> " ><strong><c:out value="${u.unitName}"></c:out></strong></a>
	...
	<c:forEach items="${compsList}" var="d">
	 <c:if test="${c.projectId==d.projectId}"> 
	  <input name=componentId    id="componentId"  type=hidden  value="<c:out value="${d.componentId}"></c:out>" />
		...
		<li><a class="mycomps"  onclick="getcompinputdata(<c:out value="${c.projectId}"></c:out>,<c:out value="${d.componentId}"></c:out>,<c:out value="${u.unitId}"></c:out>)"  href="javascript:void(0);"><c:out value="${d.componentName}"></c:out></a>
	...
	<div class="panel panel-primary  col-md-12">
			<div class="panel-heading">
				<h3 class="panel-title">Edit/Delete Component Input</h3>
			</div>
		...
		<div class="col-md-2">
				<a href="../comptsimis/delCompInput">	<input type="button" class="btn btn-1 btn-primary center-block"
	...
	<!-- <div class="w3layouts_vertical_tab resp-tab-content resp-tab-content-active  col-md-8" style="display:block"> -->
		...
		<c:if test="${cplist!=null}">
								...
								<div class="row show-grid" >
										...
										<div class="col-md-2"  style="color: blue;">Unit Name: </div>
										<div class="col-md-2" style="color: red;"><c:out value="${cplist[0].unitName}"></c:out> </div>
										<div class="col-md-2" style="color: blue;">Project Name:</div>
										<div class="col-md-2" style="color: red;"><c:out value="${cplist[0].projectName}"></c:out> </div>
										
										<div class="col-md-2" style="color: blue;">Component Name:</div>
										<div class="col-md-2" style="color: red;"><c:out value="${cplist[0].componentName}"></c:out> </div>
										
										
										</div>
								
								
									<c:forEach items="${cplist}" var="d" varStatus="count">
									 <br></br>
									
									<div class="row show-grid" style="border: 2px solid ; border-color:maroon; ">
										
											<div class="row show-grid" style=" background: #aed6f1">
										
										<div class="col-md-2">S.No</div>
										<div class="col-md-2">Device Name</div>
										<div class="col-md-3">connected Load</div>
										<div class="col-md-3">Actual Load</div>
										<div class="col-md-2">RPM</div>
										
										</div>
												<div class="row show-grid" style="color: blue">
												
									
										<div class="col-md-2"><c:out value="${count.count}"></c:out> </div>
										<div class="col-md-2"><c:out value="${d.deviceName}"></c:out> </div>
										<div class="col-md-3"><c:out value="${d.connectedLoad}"></c:out> </div>
										<div class="col-md-3"><c:out value="${d.actualLoad}"></c:out> </div>
										<div class="col-md-2"><c:out value="${d.rpm}"></c:out> </div>
										</div>
										<div class="row show-grid"  style=" background: #DCDCDC" >
										...
										<div class="col-md-2"><c:out value="${d.motorMake}"></c:out> </div>
										<div class="col-md-2"><c:out value="${d.motorType}"></c:out> </div>
										<div class="col-md-2"><c:out value="${d.pumpCapacityName}"></c:out> </div>
										<div class="col-md-2"><c:out value="${d.status}"></c:out> </div>
										<div class="col-md-2">  <button type="button" class="btn btn-primary btn-sm" onclick='editCompMotorData(<c:out value="${d.compInfoId}"></c:out>)'><span class="glyphicon glyphicon-plus"></span></button></div>
							          <div class="col-md-2"><button type="button" class="btn btn-danger btn-sm" onclick='deleteCompMotorData(<c:out value="${d.compInfoId}"></c:out> )'><span class="glyphicon glyphicon-remove"></span></button></div>
										</div>
